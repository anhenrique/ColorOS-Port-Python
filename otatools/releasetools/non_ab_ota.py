...
""" % bcb_dev)
  ...
  # 计算每个分区的进度增量
  increment = 0.071429  # 每个分区占 7.1429%

  # 获取分区的数量
  num_partitions = len(block_diff_dict)

  # 如果分区数量小于 14，则调整每个分区的进度增量，使总进度为 1.0
  if num_partitions < 14:
      increment = 1.0 / num_partitions

  # 计算系统分区的剩余进度
  system_progress = 1.0 - (num_partitions - 1) * increment

  # 如果清除用户数据（wipe_user_data），则减少系统分区的进度
  if OPTIONS.wipe_user_data:
      system_progress -= increment

  # 创建一个进度字典，所有分区的进度初始化为 increment，系统分区的进度设置为 system_progress
  progress_dict = {partition: increment for partition in block_diff_dict}
  progress_dict["system"] = system_progress
  ...
  script.ShowProgress(increment, 10)
