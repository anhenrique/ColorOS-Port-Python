...
class ProgressTracker:
    ...
    def __init__(self, total: int, description: str = "Processing", unit: str = "items"):
        ...
        self._log_interval = 5  # Log every 5 seconds minimum
    ...
    def _maybe_log(self, message: Optional[str] = None):
    ...
    def _log_progress(self, message: Optional[str] = None):
        ...
        if self.total > 0:
            ...
            progress_msg = f"[{self.description}] {self.current}/{self.total} {self.unit} ({percentage:.1f}%) - ETA: {eta}"
            if message:
    ...
    def finish(self):
        ...
        logger.info(f"[{self.description}] Completed {self.current}/{self.total} {self.unit} in {elapsed_str}")
...
def create_progress_tracker(total: int, description: str = "Processing", unit: str = "items") -> ProgressTracker:
