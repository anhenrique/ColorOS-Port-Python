...
class ModifierPlugin(ABC):
    ...
    def run_command(
        ...
        """
        self.logger.debug(
            f"Executing command: {' '.join(cmd) if isinstance(cmd, list) else cmd}"
        )
...
class PluginManager:
    ...
    def _group_by_priority(
        self, plugins: List[ModifierPlugin]
    ) -> Dict[int, List[ModifierPlugin]]:
    ...
    def _execute_single_plugin(
        ...
        try:
            ...
            if not plugin.check_prerequisites():
                self.logger.info(
                    f"Skipping plugin {plugin.name}: prerequisites not met"
                )
                return None
            ...
            try:
                if self._transaction_manager:
                    with self._transaction_manager.transaction(plugin.name) as txn:
                        ...
                        if timeout:
                            success: Optional[bool] = self._execute_with_timeout(
                                plugin, timeout
                            )
                        else:
                        ...
                        if success:
                            ...
                            self.logger.warning(
                                f"Plugin {plugin.name} returned failure"
                            )
                        return success
                else:
                    timeout = plugin.timeout
        ...
        for priority in priorities:
            ...
            if parallel_safe and len(group) > 1 and not self._dry_run:
                ...
                with ThreadPoolExecutor(
                    max_workers=min(self._max_workers, len(group))
                ) as executor:
                    futures = {
                    ...
                    for future in as_completed(futures):
                        ...
                        try:
                            ...
                            if skip_reason == "prerequisites":
                                ...
                                self.logger.error(
                                    f"Plugin {plugin.name} failed: {result}"
                                )
                            else:
                                results[plugin.name] = result
                                # Capture buffered logs
                                if buffer_handler:
                                    plugin_logs[plugin.name] = (
                                        buffer_handler.buffer.getvalue()
                                    )
                        except Exception as e:
                            self.logger.error(
...
def create_backup_hook_factory(
    get_paths_func: Callable[[], List[Path]], action: str = "modify"
):
...
def load_plugins_from_config(
    config: Dict[str, Any], manager: PluginManager
) -> PluginManager:
